    ...
    >
      ...
      >
        ...
        <button onClick={onBack} aria-label="رجوع" style={topBackButtonStyle}>
          <IconBack size={29} color="#fff" />
        </button>

        <div style={{ position: "absolute", top: "calc(18px + env(safe-area-inset-top))", right: 16, display: "flex", gap: 9 }}>
          <button onClick={shareCar} aria-label="مشاركة" style={topActionButtonStyle}>
            <IconShareUpload size={24} color="#fff" strokeWidth={2.15} />
          </button>
          <button onClick={onLike} aria-label="مفضلة" style={topActionButtonStyle}>
            <IconLike size={27} active={liked} color="#fff" />
          </button>
        </div>
      ...
      <section style={{ padding: "14px 18px 12px", background: "linear-gradient(180deg,#060607 0%,#050506 100%)", borderBottom: "1px solid rgba(255,255,255,.055)" }}>
        <h1 style={{ margin: 0, fontSize: "clamp(23px,5.8vw,29px)", lineHeight: 1.15, fontWeight: 950, letterSpacing: ".1px" }}>{car.name}</h1>
        ...
        <div style={{ marginTop: 8, display: "flex", alignItems: "center", flexWrap: "wrap", gap: 6, color: "rgba(255,255,255,.72)", fontSize: 13.2, lineHeight: 1.3, fontWeight: 750 }}>
          ...
          <span style={{ color: "rgba(255,255,255,.43)" }}>({trips} رحلة)</span>
        </div>
      ...
      <section style={{ padding: "13px 18px 20px" }}>
        <div style={smallCardStyle}>
          <div style={{ padding: "13px 14px 2px" }}>
            <h2 style={{ margin: 0, fontSize: 17, fontWeight: 950 }}>رحلتك</h2>
            <p style={{ margin: "4px 0 0", color: "rgba(255,255,255,.42)", fontSize: 11.4, fontWeight: 700 }}>معلومات الحجز الأساسية</p>
          </div>

          <div style={{ padding: "3px 14px 0" }}>
            <DateLine icon={<IconCalendar size={17} color="currentColor" />} title="تاريخ الاستلام" value={formatArabicDate(pickupDate)} onEdit={() => setEditingDates((v) => !v)} />
            <DateLine icon={<IconCalendar size={17} color="currentColor" />} title="تاريخ الإرجاع" value={formatArabicDate(returnDate)} onEdit={() => setEditingDates((v) => !v)} />
            {editingDates && (
          ...
          <div style={{ padding: "10px 14px", borderTop: "1px solid rgba(255,255,255,.06)" }}>
            <div style={{ color: "rgba(255,255,255,.45)", fontSize: 11.2, fontWeight: 850, marginBottom: 7 }}>هل تحتاج سائق؟</div>
          ...
          <div style={{ padding: "0 14px 13px", display: "grid", gap: 8 }}>
            <label style={labelStyle}>
              <span>اسمك ولقبك في الوثائق الرسمية</span>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <strong style={{ fontSize: 13.6, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{agency.name}</strong>
      ...
      <div style={bottomBarStyle}>
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ color: "rgba(255,255,255,.42)", fontWeight: 700, fontSize: 10.8 }}>الإجمالي {formatDzd(total)} · {days} يوم</div>
        </div>
        <button onClick={() => setBookingSheet(true)} style={bookButtonStyle}>حجز الآن</button>
      </div>
      ...
      {bookingSheet && (
        <div style={sheetBackdropStyle} onClick={() => setBookingSheet(false)}>
          <div style={bookingSheetStyle} onClick={(e) => e.stopPropagation()} onTouchStart={onSheetTouchStart} onTouchEnd={onSheetTouchEnd}>
            ...
            <h3 style={{ margin: 0, color: "#fff", fontSize: 17, fontWeight: 950 }}>اختر طريقة الحجز</h3>
            <p style={{ margin: "5px 0 14px", color: "rgba(255,255,255,.48)", fontSize: 12, fontWeight: 750 }}>يمكنك الحجز مباشرة أو حفظ الرحلة للعودة إليها لاحقًا.</p>
            <button type="button" onClick={openWhatsappBooking} style={{ ...sheetOptionStyle, background: "rgba(34,197,94,.12)", borderColor: "rgba(34,197,94,.22)" }}>
              <span style={{ ...sheetIconStyle, background: "linear-gradient(135deg,#22C55E,#16A34A)" }}><IconWhatsapp size={18} color="#fff" /></span>
              <span><strong>الحجز عبر واتساب</strong><small>إرسال تفاصيل الرحلة للوكالة</small></span>
            </button>
            <button type="button" onClick={confirmBooking} style={sheetOptionStyle}>
              <span style={sheetIconStyle}><IconCalendar size={18} color="#fff" /></span>
              <span><strong>الحجز عبر المنصة</strong><small>ستقوم وكالة السيارة بالاتصال بك قريبًا</small></span>
            </button>
            <button type="button" onClick={saveTripOnly} style={sheetOptionStyle}>
              <span style={sheetIconStyle}><IconPin size={18} color="#fff" /></span>
              <span><strong>حفظ الرحلة</strong><small>إضافتها إلى صفحة عرض رحلاتي</small></span>
            </button>
          </div>
        </div>
      )}
      ...
      {bookingDone && (
        <div style={modalBackdropStyle} onClick={() => setBookingDone(false)}>
          <div style={successModalStyle} onClick={(e) => e.stopPropagation()}>
            ...
            <h3 style={{ margin: 0, fontSize: 17, fontWeight: 950 }}>تم إرسال طلب الحجز</h3>
            <p style={{ margin: "8px 0 0", color: "rgba(255,255,255,.65)", fontSize: 12.8, lineHeight: 1.7, fontWeight: 750 }}>ستقوم وكالة السيارة بالاتصال بك قريبًا لتأكيد التفاصيل.</p>
            <div style={{ marginTop: 14, display: "grid", gap: 8 }}>
              <a href={whatsappUrl()} target="_blank" rel="noreferrer" style={{ ...bookButtonStyle, minHeight: 44, textDecoration: "none", display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>الحجز عبر واتساب</a>
              <button type="button" onClick={() => navigate("/trips")} style={secondaryButtonStyle}>عرض رحلاتي</button>
            </div>
          </div>
        </div>
      )}
...
const bottomBarStyle = {
  ...
  zIndex: 120,
  display: "grid",
  gridTemplateColumns: "1fr minmax(128px, 40vw)",
  alignItems: "center",
  gap: 9,
