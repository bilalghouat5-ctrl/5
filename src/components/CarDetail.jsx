    ...
    >
      ...
      <div style={bottomBarStyle}>
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ color: "rgba(255,255,255,.30)", fontWeight: 700, fontSize: 9.6 }}>يشمل رسوم المنصة 5%: {formatDzd(platformFee)}</div>
        </div>
        <button onClick={() => { if (!getCurrentUser()) setGuestAuthSheet(true); else openWhatsappBooking(); }} aria-label="الحجز عبر واتساب" style={{ ...bookButtonStyle, minWidth: 48, width: 48, height: 48, padding: 0, borderRadius: 999, background: "linear-gradient(135deg,#25D366,#128C7E)", color: "#fff", boxShadow: "0 12px 26px rgba(37,211,102,.28)", border: "1px solid rgba(255,255,255,.12)" }}><IconWhatsapp size={22} color="currentColor" /></button>
          ...
          <div style={bookingSheetStyle} onClick={(e) => e.stopPropagation()} onTouchStart={onSheetTouchStart} onTouchEnd={onSheetTouchEnd}>
            ...
            <div style={sheetContentStyle}>
              <div style={{ ...smallCardStyle, padding: 13, marginBottom: 13, background: "rgba(255,255,255,.045)" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "22px 1fr 30px", alignItems: "center", gap: 9, padding: "9px 0", borderBottom: "1px solid rgba(255,255,255,.06)" }}>
                  <span style={{ color: "rgba(255,255,255,.68)", display: "grid", placeItems: "center" }}><IconCalendar size={16} color="currentColor" /></span>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ color: "rgba(255,255,255,.42)", fontSize: 10.5, fontWeight: 800 }}>تاريخ الاستلام وتاريخ الإرجاع</div>
                    <div style={{ color: "#fff", fontSize: 12.2, fontWeight: 850, marginTop: 2, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      <span>{formatArabicDate(pickupDate)}</span>
                      <span style={{ color: "rgba(255,255,255,.35)" }}>←</span>
                      <span>{formatArabicDate(returnDate)}</span>
                    </div>
                  </div>
                  <button type="button" onClick={() => setEditingDates((v) => !v)} aria-label="تعديل التواريخ" style={{ width: 28, height: 28, borderRadius: 9, border: "1px solid rgba(167,139,250,.2)", background: "rgba(124,58,237,.13)", color: "#C4B5FD", fontSize: 12, fontWeight: 950, display: "grid", placeItems: "center" }}>✎</button>
                </div>
                {editingDates && (
                ...
                <div style={{ display: "grid", gap: 8, margin: "10px 0 10px" }}>
                  <input value={clientName} onChange={(e) => setClientName(e.target.value)} placeholder="الاسم الكامل" style={fieldStyle} />
                  <input value={clientPhone} onChange={(e) => setClientPhone(e.target.value)} placeholder="رقم الهاتف" inputMode="tel" style={fieldStyle} />
                </div>
                ...
                <div style={{ marginTop: 12, paddingTop: 10, borderTop: "1px solid rgba(255,255,255,.07)", display: "grid", gap: 5, color: "rgba(255,255,255,.68)", fontSize: 11, fontWeight: 800 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", gap:10 }}><span>السعر الأساسي</span><b style={{ color:"#fff" }}>{formatDzd(subtotal)}</b></div>
                  <div style={{ display:"flex", justifyContent:"space-between", gap:10 }}><span>رسوم المنصة 5%</span><b style={{ color:"#fff" }}>{formatDzd(platformFee)}</b></div>
                  <div style={{ display:"flex", justifyContent:"space-between", gap:10, color:"#C4B5FD" }}><span>الإجمالي</span><b>{formatDzd(total)}</b></div>
                </div>
              </div>
