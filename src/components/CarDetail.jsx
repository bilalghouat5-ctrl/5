  ...
  return (
    ...
    >
      ...
      <div style={bottomBarStyle}>
        ...
        <button onClick={() => { if (!getCurrentUser()) setGuestAuthSheet(true); else openWhatsappBooking(); }} aria-label="الحجز عبر واتساب" style={{ ...bookButtonStyle, minWidth: 46, width: 46, padding: 0, background: "rgba(34,197,94,.16)", color: "#22C55E", boxShadow: "none", border: "1px solid rgba(34,197,94,.28)" }}><IconWhatsapp size={20} color="currentColor" /></button>
          ...
          <div style={bookingSheetStyle} onClick={(e) => e.stopPropagation()} onTouchStart={onSheetTouchStart} onTouchEnd={onSheetTouchEnd}>
            ...
            <div style={sheetContentStyle}>
              <div style={{ ...smallCardStyle, padding: 13, marginBottom: 13, background: "rgba(255,255,255,.045)" }}>
                ...
                <DateLine icon={<IconCalendar size={16} color="currentColor" />} title="تاريخ الاستلام" value={formatArabicDate(pickupDate)} onEdit={() => setEditingDates((v) => !v)} />
                <DateLine icon={<IconCalendar size={16} color="currentColor" />} title="تاريخ الإرجاع" value={formatArabicDate(returnDate)} onEdit={() => setEditingDates((v) => !v)} />
                {editingDates && (
