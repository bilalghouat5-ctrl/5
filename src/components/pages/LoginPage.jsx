...
import { signIn, resetPassword, demoSignIn } from "../../lib/supabase.js";
...
export function LoginPage({ onBack, onGoRegister, onGoGoogle, onGoApple, onSuccess } = {}) {
  ...
  const [email, setEmail]       = useState("demo@driverent.dz");
  ...
  async function handleLogin() {
    ...
    setError(null);
    try {
      if (email.trim().toLowerCase() === "demo@driverent.dz" && password === "123456") {
        demoSignIn("demo");
      } else {
  ...
  return (
    <AuthShell onClose={_back}>
      ...
      <div style={{ padding: "22px 22px max(34px, env(safe-area-inset-bottom))", width: "100%", boxSizing: "border-box" }}>
        ...
        <div style={{margin:"0 auto 18px",padding:"10px 12px",borderRadius:14,background:"rgba(124,58,237,.10)",border:"1px solid rgba(167,139,250,.22)",color:"rgba(255,255,255,.7)",fontSize:12,lineHeight:1.7,textAlign:"center"}}>
          حساب تجريبي جاهز: <span style={{direction:"ltr",display:"inline-block",color:"#C4B5FD",fontWeight:900}}>demo@driverent.dz</span> · كلمة السر <b style={{color:"#C4B5FD"}}>123456</b>
        </div>
