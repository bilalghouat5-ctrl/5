  ...
  return (
    <div
      ...
      style={{
        ...
        background: "#07080F", overflow: "hidden", fontFamily: "Cairo, system-ui, sans-serif", direction: "rtl", animation: "authBackdropIn .22s ease",
      }}
    >
      <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
        <img src={ROAD_IMG} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", opacity: .18, filter: "blur(2px) saturate(.8)", transform: "scale(1.04)" }}/>
        <div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle at 50% 12%, rgba(124,58,237,.22), transparent 34%), linear-gradient(to bottom, rgba(7,8,15,.72), rgba(7,8,15,.96))" }}/>
      </div>
      ...
      <div
        ...
        style={{
          position: "absolute", left: "50%", bottom: 0, width: "min(100%, 520px)", maxHeight: success ? "72dvh" : "90dvh",
          transform: visible ? `translate(-50%, ${dragY}px)` : "translate(-50%, 100%)",
          transition: dragY ? "none" : (visible ? "transform .54s cubic-bezier(.22,1.55,.36,1)" : "transform .28s cubic-bezier(.4,0,.6,1)"),
          borderRadius: "32px 32px 0 0", overflowY: "auto", overflowX: "hidden", WebkitOverflowScrolling: "touch",
          background: "linear-gradient(180deg,#10111E 0%,#090A14 100%)", border: "1px solid rgba(255,255,255,.08)", borderBottom: "none", boxShadow: "0 -28px 90px rgba(0,0,0,.72)",
        }}
      >
        <div style={{ position: "sticky", top: 0, zIndex: 20, height: 26, background: "linear-gradient(180deg,rgba(16,17,30,.98),rgba(16,17,30,.72))", backdropFilter: "blur(12px)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ width: 52, height: 5, borderRadius: 999, background: "rgba(255,255,255,.18)" }}/>
        </div>
        <button onClick={close} style={{ position: "absolute", top: 18, left: 16, zIndex: 30, width: 36, height: 36, borderRadius: "50%", background: "rgba(0,0,0,.42)", backdropFilter: "blur(10px)", border: "1px solid rgba(255,255,255,.13)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <IconClose size={14} color="rgba(255,255,255,.82)"/>
        </button>
        {children}
      </div>
    </div>
