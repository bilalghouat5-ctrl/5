...
import { signUp, demoSignIn } from "../../lib/supabase.js";
  ...
  return (
    <div
      ...
      style={{
        ...
        overflow: "hidden", fontFamily: "Cairo, system-ui, sans-serif", direction: "rtl",
...
export function RegisterPage({ onBack, onGoLogin, onGoGoogle, onGoApple, onSuccess } = {}) {
  ...
  async function handleRegister() {
    ...
    setError(null);
    try {
      // Demo/local registration now. Supabase can be connected later without changing the UI.
      const session = demoSignIn("demo");
      session.user.email = form.email || session.user.email;
      session.user.user_metadata.full_name = form.name || session.user.user_metadata.full_name;
      session.user.user_metadata.phone = form.phone || session.user.user_metadata.phone;
