...
export function MessagesPage() {
  ...
  if (conv) {
    return (
      <div style={{ paddingBottom: 110, minHeight: "calc(100vh - 150px)", display: "flex", flexDirection: "column", animation: "fadeUp .28s ease" }}>
  ...
  return (
    <div style={{ paddingBottom: 112, animation: "fadeUp .35s ease" }}>
      ...
      {tab === "notifications" ? (
        ...
        <div style={{ display: "grid", gap: 10 }}>
          {CONVS.map((c, i) => (
            <button key={c.id} onClick={() => setOpen(c.id)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: 13, borderRadius: 18, border: "1px solid rgba(255,255,255,.075)", background: "rgba(255,255,255,.04)", textAlign: "right", fontFamily: "inherit", animation: `fadeUp .24s ease ${i * .04}s both` }}>
