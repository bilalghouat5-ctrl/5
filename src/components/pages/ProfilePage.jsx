...
export function ProfilePage({ onLogout }) {
    ...
    return (
      <div dir={dir} style={base}>
        ...
        <section style={{ textAlign: "center", padding: "28px 0 54px" }}>
          <UserAvatar avatar={avatar} name={name} size={104} />
    ...
    return (
      <div dir={dir} style={base}>
        ...
        <section style={{ paddingTop: 30 }}>
          <button type="button" onClick={() => alert(t.soon)} style={{ width: "100%", minHeight: 64, border: "none", borderTop: "1px solid rgba(255,255,255,.08)", borderBottom: "1px solid rgba(255,255,255,.08)", background: "transparent", color: "#F7F7FA", display: "flex", alignItems: "center", gap: 12, fontFamily: "inherit", cursor: "pointer", textAlign: dir === "ltr" ? "left" : "right" }}>
            <span style={{ flex: 1, fontSize: 15.2, fontWeight: 450 }}>{t.changePhoto}</span>
            <span style={{ transform: dir === "rtl" ? "scaleX(-1)" : "none", opacity: .85 }}><IconChevronRight size={20} color="currentColor" /></span>
          </button>
          <p style={{ margin: "18px 0 34px", color: "rgba(255,255,255,.58)", fontSize: 12.8, fontWeight: 430, lineHeight: 1.65 }}>{t.photoHelp}</p>

          <label style={{ display: "block", color: "#F7F7FA", fontSize: 15.2, fontWeight: 500, marginBottom: 8 }}>{t.about}</label>
          <textarea style={{ width: "100%", height: 122, borderRadius: 11, border: "1px solid rgba(255,255,255,.18)", background: "rgba(0,0,0,.12)", color: "#fff", fontFamily: "inherit", fontSize: 13.5, padding: 12, outline: "none", resize: "none" }} />
          <p style={{ margin: "12px 0 34px", color: "rgba(255,255,255,.58)", fontSize: 12.6, fontWeight: 430, lineHeight: 1.6 }}>{t.aboutHelp}</p>

          {[t.lives, t.works, t.school, t.languages].map((label) => (
            <button key={label} type="button" style={{ width: "100%", minHeight: 56, border: "none", borderTop: "1px solid rgba(255,255,255,.08)", background: "transparent", color: "#F7F7FA", fontFamily: "inherit", fontSize: 15.2, fontWeight: 450, textAlign: dir === "ltr" ? "left" : "right", display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer" }}>
              {label}
            </button>
          ))}
          <div style={{ height: 1, background: "rgba(255,255,255,.08)" }} />
        </section>
        <div style={{ position: "sticky", bottom: 96, paddingTop: 28, background: "linear-gradient(to top,#05050A 70%,rgba(5,5,10,0))" }}>
          <button type="button" onClick={() => navigate("/profile/view")} style={{ width: "100%", minHeight: 58, border: "none", borderRadius: 12, background: "linear-gradient(135deg,#7C3AED,#5B21B6)", color: "#fff", fontFamily: "inherit", fontSize: 15.5, fontWeight: 950, cursor: "pointer" }}>{t.save}</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div dir={dir} style={base}>
        ...
        <section style={{ paddingTop: 38 }}>
          <AccountLine dir={dir} title={t.accountInfo} onClick={() => alert(t.soon)} />
          <AccountLine dir={dir} title={t.notificationSettings} onClick={() => alert(t.soon)} />
        </section>
        <section style={{ paddingTop: 30 }}>
        ...
        <section style={{ paddingTop: 30 }}>
          <AccountLine dir={dir} title={t.transmission} onClick={() => alert(t.soon)} />
        </section>
  ...
  return (
    <div dir={dir} style={{ padding: "8px 18px 126px", animation: "fadeUp .28s ease", color: "#F7F7FA" }}>
      <section style={{ display: "flex", alignItems: "center", gap: 16, padding: "8px 0 22px", borderBottom: "1px solid rgba(255,255,255,.08)", marginBottom: 20 }}>
        <UserAvatar avatar={avatar} name={name} size={66} />
      ...
      <section>
        ...
        <div style={{ padding: "0 0 16px", borderBottom: "1px solid rgba(255,255,255,.07)", marginBottom: 4 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 10 }}>
            <span style={{ width: 30, height: 30, display: "grid", placeItems: "center", color: "rgba(255,255,255,.86)" }}><IconLanguage size={22} color="currentColor" strokeWidth={1.8} /></span>
            <div style={{ flex: 1, fontSize: 15.2, fontWeight: 520 }}>{t.language}</div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, paddingInlineStart: 46 }}>
            {LANGS.map((l) => (
              <button key={l.id} onClick={() => changeLanguage(l.id)} style={{ minHeight: 35, borderRadius: 9, border: lang === l.id ? "1px solid rgba(167,139,250,.75)" : "1px solid rgba(255,255,255,.08)", background: lang === l.id ? "linear-gradient(135deg,#7C3AED,#4F46E5)" : "rgba(255,255,255,.045)", color: lang === l.id ? "#fff" : "rgba(255,255,255,.62)", fontFamily: "inherit", fontSize: 10.5, fontWeight: 900 }}>{l.short}</button>
            ))}
          </div>
        </div>
