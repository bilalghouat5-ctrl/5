...
/** Create a local demo session for UI testing without Supabase network calls */
export function demoSignIn(provider = "demo") {
  const demoUser = {
    id: `demo-${provider}`,
    email: provider === "apple" ? "demo@icloud.com" : "demo@driverent.dz",
    email_confirmed_at: new Date().toISOString(),
    app_metadata: { provider },
    user_metadata: {
      full_name: provider === "google" ? "أحمد Google" : provider === "apple" ? "أحمد Apple" : "بلال درايف",
      phone: "0555 123 456",
...
export function setAppLanguage(lang) {
  localStorage.setItem("driverent_lang", lang);
  window.dispatchEvent(new Event("driverent-language-change"));
}
