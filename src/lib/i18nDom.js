...
const UI = {
  fr: {
    ...
    "حساب تجريبي جاهز:": "Compte démo prêt :",
    ...
    "الإشعارات مفعّلة تجريبياً": "Notifications activées en mode démo",
  ...
  en: {
    ...
    "حساب تجريبي جاهز:": "Ready demo account:",
    "كلمة السر": "Password",
    ...
    "الإشعارات مفعّلة تجريبياً": "Notifications are enabled in demo mode",
...
export function tText(text, lang) {
  ...
  Object.keys(dict).sort((a,b)=>b.length-a.length).forEach((ar) => {
    if (ar && out.includes(ar)) out = out.split(ar).join(dict[ar]);
  });
  if (lang === "fr") {
    out = out
