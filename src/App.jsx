...
import { FeaturedTypeSection } from "./components/sections/FeaturedTypeSection.jsx";
...
export default function App() {
    ...
    return (
      <>
        ...
        <FeaturedTypeSection
          ...
          type="van"
          title="سيارات الفان"
          sub="مساحة مريحة للعائلة، الأصدقاء، والرحلات الجماعية"
          accent="#22C55E"
          accent2="#86EFAC"
          label="VAN"
          onOpen={openCar}
        ...
        <FeaturedTypeSection
          ...
          sub="قيادة هادئة واقتصادية بتجربة عصرية"
          accent="#38BDF8"
          accent2="#A78BFA"
          label="EV"
          onOpen={openCar}
  ...
  return (
    <div style={{minHeight:"100vh",background:"#07080F",color:"#F1F5F9",overscrollBehavior:"contain"}}>
      ...
      {/* ── Page content ─────────────────────────────────────────────────── */}
      <div style={isCarDetailRoute
        ? {maxWidth:"none", margin:0, padding:0}
        : {maxWidth:"var(--app-max-width, 900px)",margin:"0 auto",padding:"clamp(14px,3vw,24px) var(--app-padding, 14px)"}
      }>
        {renderPage()}
      </div>

      {/* ── Bottom Nav ───────────────────────────────────────────────────── */}
      {!isCarDetailRoute && <div className="bottom-nav" style={{position:"fixed",bottom:0,left:0,right:0,background:"rgba(7,8,15,.96)",backdropFilter:"blur(20px)",borderTop:"1px solid rgba(255,255,255,.07)",display:"flex",justifyContent:"space-around",padding:"10px 0 max(10px,env(safe-area-inset-bottom))",zIndex:99}}>
        {[
      ...
      {showPwaPrompt && !isCarDetailRoute && <PWAInstallPrompt onClose={()=>{ localStorage.setItem("driverent_pwa_closed","1"); setShowPwaPrompt(false); }}/>} 
    </div>
  );
}
