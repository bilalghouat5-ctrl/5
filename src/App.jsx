...
export default function App() {
  ...
  const tx = {
    ar: { explore:"استكشف", favorites:"المفضلة", trips:"رحلاتي", messages:"رسائل", profile:"حسابي", loginRequired:"سجّل الدخول للمتابعة", loginSub:"هذه الصفحة مخصصة للمستخدمين المسجلين.", login:"تسجيل الدخول", register:"إنشاء حساب", nearby:"قريب مني", suggest:"اقترح لي سيارة", search:"ابحث: سيارة، ولاية، نوع..." },
    fr: { explore:"Explorer", favorites:"Favoris", trips:"Trajets", messages:"Messages", profile:"Compte", loginRequired:"Connectez-vous pour continuer", loginSub:"Cette page est réservée aux utilisateurs connectés.", login:"Connexion", register:"Créer un compte", nearby:"Près de moi", suggest:"Suggérer une voiture", search:"Rechercher: voiture, wilaya, type..." },
    en: { explore:"Explore", favorites:"Favorites", trips:"Trips", messages:"Messages", profile:"Account", loginRequired:"Sign in to continue", loginSub:"This page is for signed-in users.", login:"Sign in", register:"Create account", nearby:"Near me", suggest:"Suggest a car", search:"Search: car, wilaya, type..." }
  }[appLang] || {};
  ...
  const renderPage = () => {
    ...
    if (["/favorites", "/trips", "/messages", "/profile"].includes(pathname) && !isLoggedIn) {
      return (
  ...
  return (
    <div style={{minHeight:"100vh",background:"#07080F",color:"#F1F5F9",overscrollBehavior:"contain"}}>
      ...
      {/* ── Bottom Nav ───────────────────────────────────────────────────── */}
      {!isFullDetailRoute && <div className="bottom-nav" style={{position:"fixed",bottom:0,left:0,right:0,background:"rgba(7,8,15,.96)",backdropFilter:"blur(20px)",borderTop:"1px solid rgba(255,255,255,.07)",display:"flex",justifyContent:"space-around",padding:"10px 0 max(10px,env(safe-area-inset-bottom))",zIndex:99}}>
        {[
