...
import { Search, SlidersHorizontal, MapPin, Calendar, Star, ShieldCheck, Fuel, UserPlus, Heart, X, Check, Wallet, ChevronLeft, ChevronRight, MessageSquare, Info, Plane, Sparkles, Crown, Zap, Building, Award, Compass, Globe, Navigation, Train, Bed, Wifi, CarFront } from "lucide-react";
...
export default function ExploreTab({ cars, user, onUpdateUser, onAddBooking, onAddNotification }: ExploreTabProps) {
  ...
  const luxuryCars = cars.filter(c => c.category === "luxury" && c.isAvailable && c.fuel !== "electric" && cityMatch(c));
  ...
  return (
    <div id="explore-tab-view" className="flex flex-col h-full bg-black text-white overflow-y-auto no-scrollbar pb-24 select-none relative">
      
      {/* Search Header Container (Pure Luxurious Dark) */}
      <div id="explore-hero-header" className="px-4 pt-3.5 pb-3 shrink-0 bg-black sticky top-0 z-20">
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-2">
            ...
            >
              <span className="text-[#a1a1aa] text-xs font-semibold font-sans">
                Search anywhere
              </span>
          ...
          <div id="turo-category-pills" className="flex gap-2 overflow-x-auto no-scrollbar py-0.5 select-none scroll-smooth">
            {[
              { id: "الكل", label: "All", icon: CarFront },
              ...
              return (
                <button
                  ...
                  className={`px-3.5 py-2.5 rounded-[12px] text-xs font-bold whitespace-nowrap transition-all duration-200 border flex items-center gap-2 h-9 ${
                    isSelected 
                ...
                >
                  {IconComp && (
                    <IconComp className={`w-3.5 h-3.5 ${isSelected ? "text-black stroke-[2.3]" : "text-zinc-400 stroke-[2.1]"}`} />
                  )}
          ...
          >
            <div className="flex items-center gap-3.5">
              ...
              <div className="text-right">
                ...
                <p className="text-[10px] text-zinc-500 font-bold mt-1">ولاية باتنة، 05، الجزائر • Batna, DZ</p>
              </div>
            </div>
                    ...
                    <div className="p-4 text-right">
                      ...
                      <div className="flex gap-1.5 flex-wrap pt-3">
                        <span className="bg-purple-950/40 border border-purple-900/60 text-purple-300 text-[8.5px] font-extrabold px-2.5 py-1 rounded-[6px] flex items-center gap-1">
                          <Award className="w-3.5 h-3.5 text-purple-400" /> مضيف متميز تورو
                        </span>
          ...
          <div className="space-y-7">
            ...
            {currentPill === "الكل" && (
              <div className="space-y-3.5">
                ...
                {/* 2 Column Card Grid matching Screenshot precisely */}
            ...
            {currentPill === "Monthly" && (
              <div className="space-y-4" dir="rtl">
                ...
                <div className="flex flex-col gap-4">
                  {vanCars.map(renderCarCard)}
                </div>
              </div>
            )}

            {/* Nearby category selection rendering */}
            {currentPill === "Nearby" && (
              <div className="space-y-4" dir="rtl">
                <div className="flex justify-between items-center text-right font-sans">
                  <div>
                    <h3 className="text-xs font-black text-white tracking-wide uppercase">سيارات متوفرة بالقرب منك</h3>
                    <p className="text-[10px] text-zinc-500 mt-1">سيارات كراء جاهزة للتسليم الفوري على بُعد أقل من 2 كم من موقعك</p>
                  </div>
                </div>
                <div className="flex flex-col gap-4">
                  {suvCars.map(renderCarCard)}
                </div>
              </div>
            )}
      ...
      <AnimatePresence>
        {selectedCar && (
          <div id="car-details-overlay" className="absolute inset-0 z-50 flex items-end justify-center bg-black/90 backdrop-blur-md">
            
            {/* Modal Box */}
            <motion.div
              layoutId={`car-card-${selectedCar.id}`}
              className="bg-[#000000] w-full max-w-md rounded-t-3xl overflow-hidden max-h-[94vh] flex flex-col shadow-2xl relative border-t border-zinc-900 text-right font-sans"
            >
              {/* Image banner with custom action overlays */}
              <div className="relative h-64 w-full shrink-0 bg-neutral-950">
                <img 
                ...
                <div className="absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-[#000000] to-transparent" />
                
                {/* Header Action Row (Close, Share, Favorite) */}
                <div className="absolute top-4 inset-x-4 flex justify-between items-center" dir="rtl">
                  <div className="flex gap-2">
                    <button
                      onClick={() => setSelectedCar(null)}
                      className="w-9 h-9 rounded-full bg-black/60 backdrop-blur-md flex items-center justify-center text-white border border-zinc-800/80 hover:bg-black/90 active:scale-90 transition-all cursor-pointer"
                      title="إغلاق"
                    >
                  ...
                  <div className="flex gap-2">
                    {/* Share action button wrapper */}
                    <button
                      onClick={() => alert("تم نسخ رابط المركبة بنجاح!")}
                      className="w-9 h-9 rounded-full bg-black/60 backdrop-blur-md flex items-center justify-center text-white border border-zinc-800/80 hover:bg-black/90 active:scale-90 transition-all cursor-pointer"
                      title="مشاركة"
                    >
                    ...
                    <button
                      ...
                      title="المفضلة"
                    >
                      <Heart className={`w-4 h-4 ${favorites.includes(selectedCar.id) ? "fill-rose-500 text-rose-500" : "text-zinc-100"}`} />
                    </button>
                  </div>
                </div>

                {/* Overlaid Image Counter on bottom-right */}
                <div className="absolute bottom-4 right-4 bg-black/75 backdrop-blur-sm px-2.5 py-1 rounded-md text-[9px] font-extrabold text-zinc-300 font-mono select-none tracking-wider border border-zinc-900/40">
                  1 من 12
                </div>
              </div>

              {/* Core Details scrollable area mimicking Turo screenshot layout */}
              <div className="px-5 py-4 overflow-y-auto no-scrollbar flex-1 pb-28 space-y-5 text-right bg-[#000000]" dir="rtl">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[12.5px] font-bold text-zinc-400 mt-1.5 flex items-center gap-1.5 justify-start">
                    <span>{selectedCar.year}</span>
                    <span className="text-zinc-655">•</span>
                    <span className="font-extrabold text-[#f59e0b] flex items-center gap-0.5">
                      {selectedCar.rating.toFixed(1)} <Star className="w-3.5 h-3.5 fill-[#f59e0b] text-[#f59e0b] inline" />
                    </span>
                    <span className="text-zinc-550">({selectedCar.reviewsCount} رحلة)</span>
                  </p>
                </div>

                {/* All-Star Host badge bar */}
                <div className="flex items-center gap-2 bg-[#141416]/40 px-3 py-2 rounded-xl border border-zinc-900/60 max-w-max">
                  <Award className="w-4.5 h-4.5 text-[#5c61ec]" />
                  <span className="text-xs font-black text-zinc-300">مضيف متميز تورو</span>
                </div>

                {/* Horizontal features pill grid with square borders (Screenshot 1) */}
                <div className="grid grid-cols-2 gap-2.5 pt-1.5 text-right">
                  <div className="bg-[#121214] text-zinc-300 border border-zinc-850 rounded-xl px-3 py-2.5 flex items-center gap-2.5 text-xs font-extrabold">
                    <UserPlus className="w-4 h-4 text-zinc-400 shrink-0" />
                    <span>{selectedCar.seats} مقاعد</span>
                  </div>
                  <div className="bg-[#121214] text-zinc-300 border border-zinc-850 rounded-xl px-3 py-2.5 flex items-center gap-2.5 text-xs font-extrabold truncate">
                    <Fuel className="w-4 h-4 text-zinc-400 shrink-0" />
                    <span className="capitalize">{selectedCar.fuel === 'electric' ? 'كهربائية بالكامل' : selectedCar.fuel === 'hybrid' ? 'هجينة اقتصادية' : selectedCar.fuel === 'diesel' ? 'ديزل (مازوت فخم)' : 'بنزين ممتاز'}</span>
                  </div>
                  <div className="bg-[#121214] text-zinc-300 border border-zinc-850 rounded-xl px-3 py-2.5 flex items-center gap-2.5 text-xs font-extrabold">
                    <Compass className="w-4 h-4 text-zinc-400 shrink-0" />
                    <span>معدل اقتصادي مميز</span>
                  </div>
                  <div className="bg-[#121214] text-zinc-300 border border-zinc-850 rounded-xl px-3 py-2.5 flex items-center gap-2.5 text-xs font-extrabold truncate">
                    <SlidersHorizontal className="w-4 h-4 text-zinc-400 shrink-0" />
                    <span>{selectedCar.transmission === 'automatic' ? 'علبة تروس تلقائية' : 'علبة تروس يدوية'}</span>
                  </div>
                </div>
                ...
                {/* Interactive Trip Dates Selection Box (Screenshot 1: "Your trip") */}
                <div className="border-t border-zinc-900 pt-4">
                  <h3 className="text-[13.5px] font-black text-white tracking-wide text-right mb-3">تفاصيل رحلتك</h3>
                  <div className="bg-[#141416] border border-zinc-850 rounded-2xl p-4 flex justify-between items-center text-right">
                    <div className="flex items-center gap-3">
                      <Calendar className="w-5 h-5 text-indigo-400" />
                      <div>
                        <span className="text-[11px] font-black text-[#5c61ec] block">تواريخ استئجار المركبة</span>
                        <span className="text-[11.5px] font-extrabold text-zinc-200 mt-1 block">
                    ...
                    <button 
                      ...
                      className="w-10 h-10 rounded-full bg-zinc-900 hover:bg-zinc-800 flex items-center justify-center text-zinc-200 transition-all border border-zinc-800"
              ...
              {bookingSuccess ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-zinc-905 p-3 rounded-2xl border border-zinc-900 flex flex-col gap-2 font-semibold">
                    ...
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-zinc-500">تأمين تورو والوساطة (5%):</span>
